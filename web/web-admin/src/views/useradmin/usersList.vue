  ...
  methods: {
    ...
    */
    async successList (params) {
      ...
      let res = await this.$api.systemModule.usersManage.getList(params)
    ...
    */
    editHandle (index, row) {
      ...
      let params = row.id
	  // return this.$message.error("请配置接口!");
      this.$api.systemModule.usersManage.getDataById(params)
        .then(res => {
          if (res.data.code === this.$config.RET_CODE.SUCCESS_CODE) {
            let data = res.data.data
            //设置传给后台的数据
    ...
    */
    deleteHandle (tipsText, params, cancelTips) {
      ...
      this.$confirm(tipsText, '提示', {
        ...
        //请求删除账号接口
        this.$api.systemModule.usersManage.delete(params.ids)
          .then(res => {
            if (res.data.code === this.$config.RET_CODE.SUCCESS_CODE) {
              this.$message({
                type: 'success',
    ...
    */
    singleDeleteHandle (index, row) {
      ...
      this.deleteHandle('确定删除当前user信息?', params, '取消删除')
    },
    /*
    ...
    */
    batchDeleteHandle () {
      if (this.checkedIds.length > 0) {
        ...
        this.deleteHandle('确定批量删除当前user信息?', params, '取消删除')
      } else {
        this.$message({
          type: 'error',
          message: '请选择user'
    ...
    */
    submitAddedEditorForm () {
      ...
      if (this.handleType === 'add') {
        //添加
        this.$api.systemModule.usersManage.addUser(params)
          .then(res => {
            if (res.data.code === this.$config.RET_CODE.SUCCESS_CODE) {
        ...
        //编辑
        debugger
        this.$api.systemModule.usersManage.edit(params)
          .then(res => {
            if (res.data.code === this.$config.RET_CODE.SUCCESS_CODE) {
              this.submitLoading = false
              this.$message({
